...
% Values of i_country:
% frac_report - fraction of reported cases in all cases
% 1 - USA
% 2 - Italy
% 3 - Spain
% 4 - Germany
...
% Define scaling factor based on frac_report
frac_report = 0.5;
scaling_factor = 1/frac_report;
ActiveCases = varfun(@(x)x*scaling_factor,ActiveCases);
TotalCases = varfun(@(x)x*scaling_factor,TotalCases);
% scaling factor corrects for the influence of 
...
xlabel(sprintf('Days since %s',start_date));ylabel('Count');
saveas(gcf,strcat('figures/',model_name,'.fig'));
